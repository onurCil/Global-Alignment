...
def main():
    ...
    for i in range(len(inputs)):
        ...
        for j in range(len(inputs)):
            ...
            #print("######",inputs[j])
            if i != j and i< j:
                (ag, bg, gs, al, bl, ls) = align(inputs[i][1], inputs[j][1], sm)
                ...
                for k in range (len(ag)):
                    if (ag[k] == bg[k]):
                        counter +=1
                result = counter/len(ag)
                similaritiesRatio.append((inputs[i][0],inputs[j][0],result))
                #similaritiesRatio.append(result)
                print(result)				
                result = 0
                counter = 0
    print(similaritiesRatio)
